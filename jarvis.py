...
engine.setProperty('voice',voices[0].id) #index '0' for 'David'(male) voice index 
'1' for 'zira'(female) voice 
...
class MainThread(QThread): 
    ...
    def run_jarvis(self): 
        self.wish() 
        self.talk('Hello   I am jarvis your assistant. please tell me how can i 
help you') 
        while True: 
            self.command = self.take_Command() #Every time taking command after a 
task is done 
            print(self.command) 
...
("download a song" in self.command) or ("download song" in self.command) :  
                #commands for opening youtube, playing a song in youtube, and 
download a song in youtube 
                self.yt(self.command) #function is from line 555 
            #Interaction commands with JARVIS 
            elif ('your age' in self.command) or ('are you single'in 
self.command) or ('are you there' in self.command) or ('tell me something' in 
self.command) or ('thank you' in self.command) or ('in your free time' in 
self.command) or ('i love you' in self.command) or ('can you hear me' in 
self.command) or ('do you ever get tired' in self.command): 
                self.Fun(self.command) 
            elif 'time' in self.command :  
                self.Clock_time(self.command) 
            elif (('hi' in self.command) and len(self.command)==2) or ((('hai' in 
self.command) or ('hey' in self.command)) and len(self.command)==3) or (('hello' 
in self.command) and len(self.command)==5): 
                self.comum(self.command) 
            elif ('what can you do' in self.command) or ('your name' in 
self.command) or ('my name' in self.command) or ('university name' in 
self.command): 
                self.Fun(self.command) 
            elif ('joke'in self.command) or ('date' in self.command): 
                self.Fun(self.command) 
            #schedule commands for remembering you what is the planns of the day 
            elif ("college time table" in self.command) or ("schedule" in 
self.command): 
            ...
            #command if you don't want the JARVIS to spack until for a certain 
time 
            #Note: I can be silent for max of 10mins 
            # Eg: JARVIS keep quiet for 5 minutes  
            elif ('silence' in self.command) or ('silent' in self.command) or 
('keep quiet' in self.command) or ('wait for' in self.command) : 
            ...
            elif ('facebook' in self.command) or ('whatsapp' in self.command) or 
('instagram' in self.command) or ('twitter' in self.command) or ('discord' in 
self.command) or ('social media' in self.command): 
                self.social(self.command) 
            #command for opening your OTT platform accounts 
            #Eg: open hotstar 
            elif ('hotstar' in self.command) or ('prime' in self.command) or 
('netflix' in self.command): 
            ...
            elif ('open teams'in self.command) or ('open stream'in self.command) 
or ('open sharepoint'in self.command) or('open outlook'in self.command)or('open 
amrita portal'in self.command)or('open octave'in self.command): 
                self.college(self.command) 
            #command to search for something in wikipedia 
            #Eg: what is meant by python in wikipedia (or) search for 
"_something_" in wikipedia 
            elif ('wikipedia' in self.command) or ('what is meant by' in 
self.command) or ('tell me about' in self.command) or ('who the heck is' in 
self.command): 
                self.B_S(self.command) 
            #command for opening your browsers and search for information in 
google 
            ...
            elif ('open gmail'in self.command) or('open maps'in self.command) 
or('open calender'in self.command) or('open documents'in self.command )or('open 
spredsheet'in self.command) or('open images'in self.command) or('open drive'in 
self.command) or('open news' in self.command): 
...
: 
            ...
            #commands to open presentaion makeing tools like CANVA and GOOGLE 
SLIDES 
            elif ('slides'in self.command) or ('canva'in self.command) : 
                self.edit(self.command) 
            #Command to open desktop applications 
            #It can open : caliculator, notepad,paint, teams(aka online classes), 
discord, spotify, ltspice,vscode(aka editor), steam, VLC media player 
            elif ('open calculator'in self.command) or ('open notepad'in 
self.command) or ('open paint'in self.command) or ('open online classes'in 
self.command) or ('open discord'in self.command) or ('open ltspice'in 
self.command) or ('open editor'in self.command) or ('open spotify'in 
self.command) or ('open steam'in self.command) or ('open media player'in 
self.command): 
                self.OpenApp(self.command) 
            #Command to close desktop applications 
            #It can close : caliculator, notepad,paint, discord, spotify, 
ltspice,vscode(aka editor), steam, VLC media player 
            elif ('close calculator'in self.command) or ('close notepad'in 
self.command) or ('close paint'in self.command) or ('close discord'in 
self.command) or ('close ltspice'in self.command) or ('close editor'in 
self.command) or ('close spotify'in self.command) or ('close steam'in 
self.command) or ('close media player'in self.command): 
            ...
            elif ('where i am' in self.command) or ('where we are' in 
self.command): 
                self.locaiton() 
            ...
            #Eg: jarvis open command prompt 
                ...
                os.system('start cmd') 
            #Command for opening an instagram profile and downloading the profile 
pictures of the profile 
            #Eg: jarvis open a profile on instagram  
            elif ('instagram profile' in self.command) or("profile on instagram" 
in self.command): 
                self.Instagram_Pro() 
            #Command for opening taking screenshot 
            #Eg: jarvis take a screenshot 
            elif ('take screenshot' in self.command)or ('screenshot' in 
self.command) or("take a screenshot" in self.command): 
                self.scshot() 
            ...
            #   jarvis How to make a cake (or) jarvis how to convert int to 
string in programming  
            ...
            elif ("volume up" in self.command) or ("increase volume" in 
self.command): 
                pyautogui.press("volumeup") 
                self.talk('volume increased') 
            #command for decreaseing the volume in the system 
            #Eg: jarvis decrease volume 
            elif ("volume down" in self.command) or ("decrease volume" in 
self.command): 
                pyautogui.press("volumedown") 
                self.talk('volume decreased') 
            #Command to mute the system sound 
            #Eg: jarvis mute the sound 
            elif ("volume mute" in self.command) or ("mute the sound" in 
self.command) : 
                pyautogui.press("volumemute") 
                self.talk('volume muted') 
            #command for opening your mobile camera the description for using 
this is in the README file 
            ...
            elif ("recording" in self.command) or ("screen recording" in 
self.command) or ("voice recording" in self.command): 
                try: 
                    ...
                    self.talk("  an unexpected error occured couldn't start 
screen recording") 
             
            #command for playing a dowloaded mp3 song in which is present in your 
system 
            #Eg: Jarvis play music 
            elif 'music' in self.command: 
                music_dir = 'E:\\music' #change the song path directory if you 
have songs in other directory 
            ...
            #group => Eg: send a message to school group NOTE: mention the name 
"group" otherwise jarvis cannot detect the name 
            ...
            elif ("you can sleep" in self.command) or ("sleep now" in 
self.command): 
            ...
            #jarvis wake up 
                ...
                self.talk(" , I am not sleeping, I am in online, what can I do 
for u") 
            ...
            elif ('system condition' in self.command) or ('condition of the 
system' in self.command): 
                self.talk("checking the system condition") 
                self.condition() 
            #command for knowing the latest news 
            #Eg: jarvis tell me the news 
            elif ('tell me news' in self.command) or ("the news" in self.command) 
or ("todays news" in self.command): 
                self.talk("Please wait  , featching the latest news") 
                self.news() 
            #command for shutting down the system 
            #Eg: jarvis shutdown the system 
            elif ('shutdown the system' in self.command) or ('down the system' in 
self.command): 
    ...
    def Intro(self): 
        while True: 
            ...
            if ("wake up" in self.permission) or ("get up" in self.permission): 
                self.run_jarvis() 
            elif ("goodbye" in self.permission) or ("get lost" in 
self.permission): 
                self.talk("Thanks for using me  , have a good day") 
                sys.exit() 
    ...
    def Mobilecamra(self): 
        ...
        try: 
            ...
            URL = "http://_IP_Webcam_IP_address_/shot.jpg" #Discription for this 
is available in the README file 
            while True: 
                imag_arr = 
np.array(bytearray(urllib.request.urlopen(URL).read()),dtype=np.uint8) 
    ...
    def Covid(self,s): 
        try: 
            ...
            Cured = ss["Cured"] 
            Death = ss["Death"] 
            print(f"  the total cases in {s} are {Total}, the number of active 
cases are {Active}, and {Cured} people cured, and {Death} people are death") 
            self.talk(f"  the total cases in {s} are {Total}, the number of 
active cases are {Active}, and {Cured} people cured, and {Death} people are 
death") 
            time.sleep(5) 
    ...
    def whatsapp(self,command): 
        try: 
            ...
            if F: 
                ...
                print(hour,min) 
                if "group" in command: 
                    kit.sendwhatmsg_to_group(numberID,message,int(hour),int(min)+
 1) 
                else: 
                    kit.sendwhatmsg(numberID,message,int(hour),int(min)+1) 
                self.talk("  message have been sent") 
            if F==False: 
                self.talk(f' , the name not found in our data base, shall I add 
the contact') 
                AddOrNot = self.take_Command() 
                print(AddOrNot) 
                if ("yes" in AddOrNot) or ("add" in AddOrNot) or ("yeah" in 
AddOrNot) or ("yah" in AddOrNot): 
                    self.AddContact() 
                elif("no" in AddOrNot): 
                    self.talk('Ok  ') 
        except: 
    ...
    def Display(self): 
        ...
        ContactsFile.close() 
        ContactsFile = open("Contacts.txt","r") 
        self.talk(f"  displaying the {count} contacts stored in our data 
base")     
        for line in ContactsFile: 
            s = line.split("\"") 
            print("Name: "+s[1]) 
    ...
    def NameIntheContDataBase(self,command): 
        ...
        if bo: 
            print(f"Contact Match Found in our data base with {name} and the 
mboile number is {number}") 
            self.talk(f"  Contact Match Found in our data base with {name} and 
the mboile number is {number}") 
        else: 
            self.talk("  the name not found in our data base, shall I add the 
contact") 
            AddOrNot = self.take_Command() 
            print(AddOrNot) 
            if ("yes add it" in AddOrNot)or ("yeah" in AddOrNot) or ("yah" in 
AddOrNot): 
                self.AddContact() 
                self.talk(f' , Contact Saved Successfully') 
    ...
    def InternetSpeed(self): 
        ...
        up = up/(1000000) 
        print(dl,up) 
        self.talk(f" , we have {dl} megabytes per second downloading speed and 
{up} megabytes per second uploading speed") 
    ...
    def comum(self,command): 
        print(command) 
        if ('hi'in command) or('hai'in command) or ('hey'in command) or ('hello' 
in command) : 
    ...
    def Fun(self,command): 
        ...
        if 'your name' in command: 
            ...
            self.talk("you are studing in Amrita Vishwa Vidyapeetam, with 
batcheloe in Computer Science and Artificail Intelligence")  
        elif 'what can you do' in command: 
            self.talk("I talk with you until you want to stop, I can say time, 
open your social media accounts,your open source accounts, open google 
browser,and I can also open your college websites, I can search for some thing in 
google and I can tell jokes") 
        elif 'your age' in command: 
            self.talk("I am very young that u") 
        elif 'date' in command: 
            self.talk('Sorry not intreseted, I am having headache, we will catch 
up some other time') 
            ...
            self.talk(' , I don\'t have much to say, you only tell me someting i 
will give you the company') 
    ...
    def Cal_day(self): 
        ...
        Day_dict = {1: 'Monday', 2: 'Tuesday', 3: 'Wednesday',4: 'Thursday', 5: 
'Friday', 6: 'Saturday',7: 'Sunday'} 
    ...
    def shedule(self): 
        ...
        Week = {"monday" : "  from 7:45 to 8:45 you have maths lecture, from 8:55 
to 9:55 you have python, from 10 to 11 you have break, from 11:05 to 12:05 you 
have DBMS, from 12:05 to 1:05 you have CN, from 1:15 to 3:15 you have DBMS 
practical", 
        "tuesday" : "  from 7:45 to 8:45 you have maths lecture, from 8:55 to 
10:55 you have maths practical, from 11 to 12 you have break, from 12:05 to 1:05 
you have Data Structure,  from 1:15 to 2:15 you have DBMS", 
        "wednesday" : "  today you have lec from 11:05 to 1:05 Python, from 1:15 
to 3:15 you have Computer network practical after that you can have fun", 
        "thrusday" : "  today you have a full day of classes from 9:00 to 10:50 
you have Maths class, from 11:00 to 12:50 you have sensors class, from 1:00 to 
2:00 you have brake, and today you have english lab from 2:00", 
        "friday" : "  today you have a full day of classes from 9:00 to 9:50 you 
have Biology class, from 10:00 to 10:50 you have data structures class, from 
11:00 to 12:50 you have Elements of computing class, from 1:00 to 2:00 you have 
brake, and today you have Electronics lab from 2:00", 
        "saturday" : "  today you have a full day of classes from 9:00 to 11:50 
you have maths lab, from 12:00 to 12:50 you have english class, from 1:00 to 2:00 
you have brake, and today you have elements of computing lab from 2:00", 
        "sunday":"  today is holiday but we can't say anything when they will 
bomb with any assisgnments"} 
        if day in Week.keys(): 
            self.talk(Week[day]) 
    ...
    def OnlineClasses(self,command): 
        print(command) 
        #Keep as many "elif" statemets based on your subject Eg: I have kept a 
dummy links for JAVA and mechanics classes link of MS Teams 
        if("java" in command): 
    ...
    def B_S(self,command): 
        print(command) 
        try: 
            # ('what is meant by' in self.command) or ('tell me about' in 
self.command) or ('who the heck is' in self.command) 
    ...
    def brows(self,command): 
        ...
        if 'google' in command: 
            ...
            os.startfile('..\\..\\MicrosoftEdge.exe')#path for your edge browser 
application 
        else : 
            self.No_result_found() 
    ...
    def yt(self,command): 
        ...
        if 'play' in command: 
            ...
            self.talk("  please enter the youtube video link which you want to 
download") 
            link = input("Enter the YOUTUBE video link: ") 
            yt=YouTube(link) 
            yt.streams.get_highest_resolution().download() 
            self.talk(f"  downloaded {yt.title} from the link you given into the 
main folder") 
    ...
    def CloseApp(self,command): 
        print(command) 
        if ('calculator'in command) : 
            self.talk("okay  , closeing calculator") 
    ...
    def silenceTime(self,command): 
        print(command) 
        x=0 
        #calculating the given time to seconds from the speech commnd string 
    ...
    def locaiton(self): 
        ...
        try: 
            ...
            print(city+" "+state+" "+country+" "+tZ+" "+longitude+" "+latidute+" 
"+org) 
            self.talk(f"  i am not sure, but i think we are in {city} city of 
{state} state of {country} country") 
            self.talk(f"and  , we are in {tZ} timezone the latitude os our 
location is {latidute}, and the longitude of our location is {longitude}, and we 
are using {org}\'s network ") 
        except Exception as e: 
            self.talk("Sorry  , due to network issue i am not able to find where 
we are.") 
            pass 
    ...
    def Instagram_Pro(self): 
        ...
        time.sleep(5) 
        self.talk("  would you like to download the profile picture of this 
account.") 
...
") 
    ...
    def scshot(self): 
        ...
        self.talk("Please   hold the screen for few seconds, I am taking 
screenshot") 
        time.sleep(3) 
    ...
    #News 
    def news(self): 
        MAIN_URL_= "https://newsapi.org/v2/top
headlines?sources=techcrunch&apiKey=59aa587dab5c4dffb0f367e122fa76d9" 
        MAIN_PAGE_ = get(MAIN_URL_).json() 
        articles = MAIN_PAGE_["articles"] 
        headings=[] 
        seq = 
['first','second','third','fourth','fifth','sixth','seventh','eighth','ninth','te
 nth'] #If you need more than ten you can extend it in the list 
        for ar in articles: 
    ...
    def condition(self): 
        ...
        if percentage >=75: 
            ...
            self.talk(f"  we should connect out system to charging point to 
charge our battery") 
        elif percentage >=15 and percentage <=30: 
            self.talk(f"  we don't have enough power to work, please connect to 
charging") 
        else: 
            self.talk(f"  we have very low power, please connect to charging 
otherwise the system will shutdown very soon") 
         
    #no result found 
    def No_result_found(self): 
        self.talk('  I couldn\'t understand, could you please say it 
again.')         
...
class Main(QMainWindow): 
    ...
    def startTask(self): 
        self.ui.movie = QtGui.QMovie("G:\\Second Year\\Sem 
3\\Python\\Project\\ironman1.gif") 
        self.ui.label_2.setMovie(self.ui.movie) 
        self.ui.movie.start() 
        self.ui.movie = QtGui.QMovie("G:\\Second Year\\Sem 
3\\Python\\Project\\circle.gif") 
        self.ui.label_4.setMovie(self.ui.movie) 
        self.ui.movie.start() 
        self.ui.movie = QtGui.QMovie("G:\\Second Year\\Sem 
3\\Python\\Project\\lines1.gif") 
        self.ui.label_7.setMovie(self.ui.movie) 
        self.ui.movie.start() 
        self.ui.movie = QtGui.QMovie("G:\\Second Year\\Sem 
3\\Python\\Project\\ironman3.gif") 
...
3\\Python\\Project\\circle.gif") 
        ...
        self.ui.movie.start() 
        self.ui.movie = QtGui.QMovie("G:\\Second Year\\Sem 
3\\Python\\Project\\powersource.gif") 
...
3\\Python\\Project\\powersource.gif") 
        ...
        self.ui.movie.start() 
        self.ui.movie = QtGui.QMovie("G:\\Second Year\\Sem 
3\\Python\\Project\\ironman3_flipped.gif") 
